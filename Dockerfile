...
# SDKs, parsers) that Bun would otherwise auto-install; every direct runtime
# dependency is still installed, @duckdb/node-api included. The OTEL step below
# carries the same flag — without it, that install re-resolves the graph and
...
ENV LOGS_DIR="/var/log/oecd-mcp-server"
