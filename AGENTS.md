...
await createApp({
  ...
  title: 'oecd-mcp-server',   // display name — the hyphenated repo name, never Title Case
  instructions: 'Resolve a dataflow with oecd_search_datasets before querying it.',
});
```

`description` derives from `package.json` — don't duplicate it here. `icons` and `websiteUrl` are also accepted; set them only when the server has a value that isn't already derivable.
...
- [ ] Plugin manifests carry the **unscoped** machine name `oecd-mcp-server` on every display/machine field — `.claude-plugin/plugin.json` `name` + `mcpServers` key, `.codex-plugin/plugin.json` `name` + `interface.displayName`, `.codex-plugin/mcp.json` server key. Only the `npx -y` install arg carries the full scoped `@cyanheads/oecd-mcp-server`; an unscoped install arg is a guaranteed 404. `lint:packaging` check 10 enforces the split.
- [ ] `.codex-plugin/plugin.json` populated — `version`, `description`, `repository`, `license` from `package.json`; `interface.shortDescription` from `package.json` description
- [ ] `.claude-plugin/plugin.json` populated — `version`, `description`, `repository`, `license` from `package.json`; inline `mcpServers` entry with env vars for any required API keys
