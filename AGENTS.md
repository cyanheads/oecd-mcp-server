...
- [ ] `.codex-plugin/plugin.json` populated — `name`, `version`, `description`, `repository`, `license` from `package.json`; `interface.displayName` = unscoped package name (`oecd-mcp-server`, not the `@cyanheads/` scope); `interface.shortDescription` from `package.json` description
