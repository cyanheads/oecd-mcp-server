...
Every tool that reaches OECD declares that failure as `upstream_redirect` (Forbidden, not retryable) and reports the boundary's own sentence. Without the contract entry the refusal arrives as an undeclared code carrying no recovery hint — or, on the two tools that restate any structure-API failure as `upstream_error`, as a retryable outage advising a wait that can never clear a misconfigured origin. Both services relabel an upstream failure with a message of their own on the way up, so the sentence naming the variable is read back off the cause chain rather than rewritten per tool.
